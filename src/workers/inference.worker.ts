...
import { imageDataToTensor, tensorToImageData, downscaleImageData } from '../lib/image-utils';
...
async function runInference(sess: ort.InferenceSession, imageData: ImageData): Promise<ImageData> {
...
const worker = {
    ...
    async load(
        ...
        const buffer = await fetchModel(modelUrl, (loaded, total) => onProgress({ loaded, total }));
        ...
        const epOrder: ExecutionProvider[] =
            _preferredEP === 'wasm' ? ['wasm'] : [_preferredEP, 'wasm'];
